...
<body>
    <div>
        <h1 class="text-white text-6xl hover:text-black bg-red-600 text-center">TOP INVESTORS</h1>
    </div>
    <div class="navbar">
    ...
    <form action="" method="post" class="py-28'>
        ...
        <div class="flex-wrap main">
            <?php
            $query = "SELECT * FROM investors ORDER BY fund_raised DESC";
            $select_posts = mysqli_query($conn, $query);
            while ($row = mysqli_fetch_assoc($select_posts)) {
                $inv_name = $row['inv_name'];
                $name = $row['name'];
                $inv_email = $row['inv_email'];
                $inv_orgname = $row['inv_orgname'];
                $fund_raised = $row['fund_raised'];

            ?>
                <div class=" org max-w-sm rounded overflow-hidden shadow-lg px-16 bg-red-600 text-white mx-5 my-3">
                    <!-- <img class="w-full" src="/img/card-top.jpg" alt="Sunset in the mountains"> -->
                    <div class="px-6 py-4">
                        <div class="font-bold text-xl mb-2"><?php echo "{$inv_name}"; ?></div>
                        <div class="font-bold text-xl mb-2"><?php echo "{$name}"; ?></div>
                        <div class="font-bold text-xl mb-2"><?php echo "{$inv_email}"; ?></div>
                        <p class="text-w text-base">
                            <?php echo "{$inv_orgname}"; ?>
                        </p>
                        <p class="text-w text-base">
                            <?php echo "{$fund_raised}"; ?>
                        </p>
                    </div>
